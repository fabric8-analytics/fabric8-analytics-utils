...
def validate_and_process_date(some_date):
...
def retrieve_stack_analyses_ids(start_date, end_date):
...
def retrieve_worker_results(id_list=[], worker_list=[]):
...
def populate_deps_count(deps_count_dict, deps):
...
def retrieve_dependencies_count(stack_data):
    """Retrieve the dependencies count."""
    deps_count_dict = {}

    for data in json.loads(stack_data):
        try:
            deps = normalize_deps_list(data[0]['stack_data'][0]['user_stack_info']['dependencies'])
            deps_count_dict = populate_deps_count(deps_count_dict, deps)
...
def retrieve_unknown_licenses_count(stack_data):
